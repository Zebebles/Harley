...
module.exports = class Choose extends DBF.Command{
    ...
    run(params = {"msg": msg, "args": args, "user": user}){ //all the code for your command goes in here.
        ...
        if(!args || args == "") return msg.channel.send("Usage: `" + msg.guild.prefix + "choose opt1,opt2,opt3`");
        let opts = args.match(/[\w\d \'\"\-\_]*/g).filter(s => s != "");
        if(opts.length < 2) return msg.channel.send("Usage: `" + msg.guild.prefix + "choose opt1,opt2,opt3`");
		var min = Math.ceil(0);
		var max = Math.floor(opts.length-1);
		var choice = Math.floor(Math.random() * (max - min+1)) + min;
        msg.channel.send("I choose `" + opts[choice] + "`");
     }
}