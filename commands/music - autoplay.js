...
module.exports = class Hello extends DBF.Command{
    ...
    run(params = {msg, args}){
        ...
        if(msg.guild.playlist.auto)
            msg.channel.send("Related songs will no longer be played when the queue is empty! :)").catch(err => console.log(err));
        else
            msg.channel.send("Related songs will now be played when the queue is empty! c:").catch(err => console.log(err));
