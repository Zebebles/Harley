...
module.exports = class Remove extends DBF.Command{
    ...
    run(params = {msg, args}){
        ...
        if(playlist.queue.length == 1)
            return msg.channel.send("There are no songs in the queue to remove.");
        let track;
        if(!args.match(/\d+(-\d+)?/g) || args.replace(/\d+(-\d+)?/g,"") != "") //if args is not a number or a range of numbers
            ...
            if(args.match(/\d+-\d+/g)) //if args is a range of numbers
            {
                ...
                if(start < 1 || end > msg.guild.playlist.queue.length-1)
                    return msg.channel.send("Please provide a range between `1** and `" + (msg.guild.playlist.queue.length-1) + "`")
        ...
        if(!track)
            return msg.channel.send("Couldn't find a track to remove under `" + args.substr(0, 100) + "`");
        else if(track < 1 || track > msg.guild.playlist.queue.length-1)
            return msg.channel.send("Please provide a queue position between `1` and `" + (msg.guild.playlist.queue.length-1) + "`")
